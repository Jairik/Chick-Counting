...
def get_box_features(
    ...
    Returns:
        np.ndarray: Extracted features (e.g., mean temperature, variance, etc.)
    '''
...
def save_model_results(
    temp_data: np.ndarray,  # The raw temperature data for the bounding box
    ...
    file_save_path_img = f"thermal_model_results/frame_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png"  # Unique filepath for each frame
    file_save_path_features = f"thermal_model_results/features.csv"  # Filepath for all tested features
    
    # Normalize the temperature data, add a text label, and save it
    norm_temp_data = cv2.normalize(temp_data, None, 0, 255, cv2.NORM_MINMAX).astype(np.uint8)
    cv2.putText(norm_temp_data, f'Predicted Count: {pred}', (10, norm_temp_data.shape[0] - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255,), 1, cv2.LINE_AA)
    cv2.imwrite(file_save_path_img, norm_temp_data)  # Save the image
